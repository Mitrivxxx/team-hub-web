...
## Do
- Use API through gateway (`/api/...`) with `withCredentials` when required.
...
- In dev, keep proxy `/api` on `https://localhost:5000`.
- In docker (Production), serve frontend on `https://localhost:4200` (`team-hub-web-prod`); HTTP on :4200 redirects to HTTPS; certs from `./certs` (prefer `./scripts/setup-certs.sh` / mkcert).
...
## Don't
- Do not point frontend directly at auth service URL when traffic should go through gateway.
